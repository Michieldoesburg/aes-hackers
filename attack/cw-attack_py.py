...
def main():
    ...
    #     for j in range(16):
    #         print("{:3d} ".format(ord(plaintexts[i][j])), end='')
    #     print("")

    bestguess = [0] * NUM_OF_SUBKEYS
    for bnum in range(0, NUM_OF_SUBKEYS):
        cpaoutput = [0] * KEY_RANGE
        maxcpa = [0] * KEY_RANGE
        for kguess in range(0, KEY_RANGE):
            print("Subkey {:2d}, hyp = {:02x}: ".format(bnum, kguess), end='')
            ...
            for tnum in range(0, numtraces):
                hyp[tnum] = HW[intermediate(ord(plaintexts[tnum][bnum]), kguess)]
